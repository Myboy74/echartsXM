...
$.getJSON('bigScreen/注册党员年龄分布图.json', function(res) {
	var obj=[]
	for (var i = 0; i < res.data.length; i++) {
		res.da
		obj.push({name:res.data[i],value:res.value[i]});
	}
	console.log(obj)
    option_gyt_age.series[0].data=obj;
...
$.getJSON('bigScreen/注册党员性别分布图.json', function(res) {
	var obj=[]
	for (var i = 0; i < res.data.length; i++) {
		res.da
		obj.push({name:res.data[i],value:res.value[i]});
...
// 党员性别分布
function w_sexsl(){
var myChart_age = echarts.init(document.getElementById('ls_ex'));
var option_gyt_age = {
//  color: ['#ebe20c', '#126413', '#1389d6', '#ff4200', '#55d7ff'],
	width:400,
    title: {
    ...
    series: [
        {
          ...
          radius: ['40%', '60%'],
           center: ['50%', '60%'],
          color: ['#4b89dc', '#967bdc','#00ff8a','#00d8ff','#0096ff','#0048ff',],
            label: {
             normal: {
                formatter: '{b}:{c}\n占比{d}%',
                textStyle: {
...
$.getJSON('bigScreen/注册党员学历分布图.json', function(res) {
	var obj=[]
	for (var i = 0; i < res.data.length; i++) {
		res.da
		obj.push({name:res.data[i],value:res.value[i]});
...
$.getJSON('bigScreen/注册党员岗位分布图.json', function(res) {
	var obj=[]
	for (var i = 0; i < res.data.length; i++) {
		res.da
		obj.push({name:res.data[i],value:res.value[i]});
